...
#pragma warning restore OPENAI001
            ...
            else if (input.Role == ChatRole.Assistant)
            {
                ...
                StringBuilder reasoningContentBuild = new();
                ...
                foreach (var content in input.Contents)
                {
                    switch (content)
                    {
                        ...
                        case TextReasoningContent reasoning_content:
                            reasoningContentBuild.Append(reasoning_content.Text);
                            break;
                ...
                string textContent = string.Join(string.Empty, contentParts?.Select(x => x.Text) ?? []);
                string reasoningContent = reasoningContentBuild.ToString();

                AssistantChatMessage message = new(textContent, reasoningContent);
...
#pragma warning disable OPENAI001 // WebSearchOptions is experimental
        result.Thinking ??= ToOpenAIThinkingOptions(options.Reasoning?.Output);
#pragma warning restore OPENAI001
...
#pragma warning restore OPENAI001
    ...
    [Experimental(DiagnosticIds.Experiments.AIOpenAIReasoning)]
    private static ChatThinkingOptions? ToOpenAIThinkingOptions(ReasoningOutput? reasoning) =>
        reasoning switch
        {
            ReasoningOutput.Full or ReasoningOutput.Summary => new ChatThinkingOptions(ChatThinkingType.Enabled),
            ReasoningOutput.None => new ChatThinkingOptions(ChatThinkingType.Disabled),
            _ => null,
        };
